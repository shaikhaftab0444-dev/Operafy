...
namespace ERP_System.Controllers
{
    ...
    public class NotificationsController : Controller
    {
        ...
        public async Task<IActionResult> Index(string? category, string? status)
        {
            ...
            lock (_lock)
            {
                var userList = _notificationStore!.Where(n =>
                    string.IsNullOrEmpty(n.TargetEmail) ||
                    n.TargetEmail.Equals(currentUser.email, StringComparison.OrdinalIgnoreCase) ||
                    (!string.IsNullOrEmpty(n.TargetRole) && n.TargetRole.Equals(currentUser.role, StringComparison.OrdinalIgnoreCase))
                ).ToList();

                unreadCount = userList.Count(n => !n.IsRead);
        ...
        public IActionResult MarkAsRead(int id, string? category, string? status)
        {
            lock (_lock)
            {
            ...
            return RedirectToAction(nameof(Index), new { category, status });
        }

        // POST: /Notifications/MarkAllAsRead
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> MarkAllAsRead(string? category, string? status)
        {
            var currentUser = await GetCurrentUserAsync();
            lock (_lock)
            {
                if (_notificationStore != null)
                {
                    ...
                    {
                        if (string.IsNullOrEmpty(item.TargetEmail) ||
                            item.TargetEmail.Equals(currentUser.email, StringComparison.OrdinalIgnoreCase) ||
                            (!string.IsNullOrEmpty(item.TargetRole) && item.TargetRole.Equals(currentUser.role, StringComparison.OrdinalIgnoreCase)))
                        {
                            item.IsRead = true;
                        }
                    }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public IActionResult ClearNotification(int id, string? category, string? status)
        {
            lock (_lock)
            {
                if (_notificationStore != null)
        ...
        private async Task SyncWithActivityLogsAsync(string currentUserEmail)
        {
            ...
            {
                // Fallback gracefully if database table query encounters issue
            }
        }
    }
}
